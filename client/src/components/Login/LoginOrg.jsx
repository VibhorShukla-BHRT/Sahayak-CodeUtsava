...
function LoginOrg() {
    ...
    const handleInput2= (e)=>{
      ...
      const handleSignIn = async(e)=>{
        ...
        try {
            const response = await fetch("api/v1/org/signin",{
            method: "POST",
            crossDomain: true,
            headers: {
    ...
    const handleRegister = async (e) => {
      ...
      try {
          const response = await fetch("api/v1/org/singup", {
          ...
          if (response.ok) {
              ...
              setUser({ name: "", email: "", password: "",phonenumber:"",employeeID:"",OrgID:"" })
              storeTokenInLS(res.data.token);
          ...
          <div className="form-container sign-up-container">
            {isSignUp && (
              <form className="sign" onSubmit={handleRegister}>
                ...
                <input className="signInput yoya" type="text" placeholder="Name"
                  name="name"
                  value={user.name}
                ...
                <input className="signInput yoya" type="text" placeholder="Email"
                  name="email"
                  value={user.email}
                  onChange={handleInput}
                  autoComplete='off' />
                <input className="signInput yoya" type="tel" placeholder="Mob. No."
                  name="phonenumber"
                  value={user.phonenumber}
                  onChange={handleInput}
                />
                <input className="signInput yoya" type="text" placeholder="Org. ID (XXXX:YYYYYY:ZZZ)"
                  name="OrgID"
                  value={user.OrgID}
                  onChange={handleInput}
                />
                <input className="signInput yoya" type="text" placeholder="Employee ID"
                  name="employeeID"
                  value={user.employeeID}
                  onChange={handleInput}
                />
                <input className="signInput yoya" type="text" placeholder="Password"
                  name="password"
                  value={user.password}
                  onChange={handleInput}
                  autoComplete='off'
